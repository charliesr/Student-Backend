...
namespace StudentApp.Backend.Common.Logic
{
    public class Log4NetLogger : IMyLog
    {
        ...
        public void Init(Type declaringType)
        {
            ...
            this.Debug(new StringBuilder(LoggingLiterals.EnteringClass).Append(declaringType.FullName));
        }
        ...
        {
            log.Error(new StringBuilder(LoggingLiterals.ExceptionThrown).Append(ex.GetType().Name).ToString(), ex);
        }
    }
}
